import { CarritoDAO } from "../dao/cart.dao.js";
import { ticketModelo } from "../dao/models/ticket.model.js";
import { ProductosDAO } from "../dao/products.dao.js";
...
export const comprarCarrito = async (req, res, next) => {
  ...
  try {
    ...
    if (!carrito) {
      ...
      return res
        .status(400)
        .json({ error: `Carrito con id ${idCarrito} no existe...!!!` });
    }
...
export const agregaProductoCarrito = async (req, res) => {
  ...
  let indiceProducto = carrito.productos.findIndex(
    (p) => p.producto._id == idProducto
  );
